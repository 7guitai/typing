        ...
        {currentSentence.split('').map((char, index) => {
          ...
          if (index < userInput.length) {
            ...
            className += userInput[index] === char 
              ? "text-green-800 font-bold"  // 正しい入力
              : "text-red-700 font-bold"    // 誤った入力
          } else if (index === userInput.length) {
            // 現在入力すべき文字
            className += "text-zinc-900 font-bold bg-yellow-200 px-1 rounded"
          } else {
            // 未入力の文字
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
      ...
      <input
        ...
        className="w-full p-4 border-2 border-gray-400 rounded-lg mb-6 font-mono text-xl focus:outline-none focus:border-blue-600 transition-colors"
      ...
      {isCompleted && (
        <div className="text-center">
          <p className="text-green-700 text-2xl font-bold mb-4">完了！</p>
          <p className="text-xl mb-4 text-gray-800">タイピング速度: {calculateWPM()} 文字/分</p>
          <button
